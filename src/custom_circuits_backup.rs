...
/// 自定义电路定义
#[derive(Debug, Clone)]
pub struct CustomCircuit<F: Field> {
    /// 电路名称
    pub name: String,
    /// 约束数量
    pub num_constraints: usize,
...
impl CircuitTemplates {
    ...
    pub fn square_root_verification<F: PrimeField>(x: F, y: F) -> CustomCircuit<F> {
        ...
        let y_idx = circuit.add_public_input(y);     // 公开输入: 平方值
    ...
    /// 二次方程验证电路: 验证 ax² + bx + c = 0
    pub fn quadratic_equation_verification<F: PrimeField>(a: F, b: F, c: F, x: F) -> CustomCircuit<F> {
        let mut circuit = CustomCircuit::new("quadratic_equation".to_string());
        
        // 系数作为公开输入
        let a_idx = circuit.add_public_input(a);
        let b_idx = circuit.add_public_input(b);
        let c_idx = circuit.add_public_input(c);
        
        // 解作为私有见证
        let x_idx = circuit.add_private_witness(x);
        
        // 中间变量
        let x_squared_idx = circuit.add_private_witness(x * x);
        let ax_squared_idx = circuit.add_private_witness(a * x * x);
        let bx_idx = circuit.add_private_witness(b * x);
        let ax_squared_plus_bx_idx = circuit.add_private_witness(a * x * x + b * x);
        let result_idx = circuit.add_private_witness(a * x * x + b * x + c);
        let zero_idx = circuit.add_public_input(F::zero());
        
        // 约束系统
        circuit.add_multiplication_constraint(x_idx, x_idx, x_squared_idx);     // x * x = x²
        circuit.add_multiplication_constraint(a_idx, x_squared_idx, ax_squared_idx); // a * x² = ax²
        circuit.add_multiplication_constraint(b_idx, x_idx, bx_idx);            // b * x = bx
        
        // 注意：加法约束需要用乘法来表示，这里简化处理
        // 在实际实现中，需要更复杂的约束系统来处理加法
        
        circuit
    }
    
    /// 哈希链验证电路: 验证 hash(hash(x)) = target
    pub fn hash_chain_verification<F: PrimeField>(initial: F, target: F, chain_length: usize) -> CustomCircuit<F> {
        let mut circuit = CustomCircuit::new("hash_chain".to_string());
        
        let initial_idx = circuit.add_private_witness(initial);
        let target_idx = circuit.add_public_input(target);
        
        let mut current_idx = initial_idx;
        
        // 简化的哈希函数: hash(x) = 3x + 1
        let hash_multiplier = F::from(3u64);
        let hash_additive = F::from(1u64);
        
        for i in 0..chain_length {
            let hash_mult_idx = circuit.add_private_witness(hash_multiplier);
            let hash_add_idx = circuit.add_private_witness(hash_additive);
            
            // 计算 3 * current
            let current_value = if i == 0 { initial } else { 
                // 这里需要从之前的计算中获取值，简化处理
                initial * hash_multiplier.pow(&[i as u64]) + hash_additive * F::from(i as u64)
            };
            
            let mult_result_idx = circuit.add_private_witness(hash_multiplier * current_value);
            let next_hash_idx = circuit.add_private_witness(hash_multiplier * current_value + hash_additive);
            
            // 约束: hash_multiplier * current = mult_result
            circuit.add_multiplication_constraint(hash_mult_idx, current_idx, mult_result_idx);
            
            current_idx = next_hash_idx;
        }
